...
the base concepts of the Symfony CMF and it get you started with it.

First of all, the Symfony CMF provides bundles which implement the common needs
when building a CMS with the Symfony Framework. Before you read further, you
should at least have a basic knowledge of the Symfony Framework. If you don't
have it, start by reading the `Symfony Framework Quick Tour`_.
...
build custom things the way you want.
...
can decide to only use the RoutingBundle or the MediaBundle.

Downloading the Symfony CMF Standard Edition
--------------------------------------------

When you want to start using the CMF for a new project, you can download the
Symfony CMF Standard Edition. The Symfony CMF Standard Edition adds the common
Symfony CMF bundles and their configuration to the `Symfony Standard Edition`_.
It also adds a very simple bundle to show some of the basic Symfony CMF
features.
...
This bar is comming from the `Create.js`_ library. The Symfony CMF integrates
