...
The ``StaticContent`` declares the basic content's structure. It's similar
to an entity/model/document declared using Symfony2's ORM solutions, and
most of it's fields are self explanatory, and are what you would expect from
a basic CMS: title, body, publishing information and a parent reference,
to accommodate a tree-like hierarchy.
